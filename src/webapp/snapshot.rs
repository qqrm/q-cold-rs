...
fn agent_start_template(root: &str) -> String {
    format!(
        "/agent_start --cwd {cwd} <track> :: codex exec \"Use the launched host-side agent \
         ...
         AGENTS.md and task logs, then do: <task>. Q-COLD-managed launches automatically guard \
         configured broad-output commands; if qcold-guard reports blocked output, rerun with a \
         narrower command. Drive the task to terminal closeout unless \
