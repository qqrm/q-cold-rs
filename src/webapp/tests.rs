...
mod tests {
    ...
    #[test]
    fn queue_run_rejects_duplicate_slug_before_persisting_retry_run() {
        let _guard = test_support::env_guard();
        let temp = tempdir().unwrap();
        std::env::set_var("QCOLD_STATE_DIR", temp.path());
        let existing_run = queue_run_fixture("existing-run", "failed", 0);
        let mut existing_item = queue_item_fixture("existing-run", "existing", 0, "pending", None);
        existing_item.slug = "shared-task".to_string();
    ...
    #[test]
    fn queue_run_allows_same_run_id_replacement_with_same_slug() {
        let _guard = test_support::env_guard();
        let temp = tempdir().unwrap();
        std::env::set_var("QCOLD_STATE_DIR", temp.path());
        let existing_run = queue_run_fixture("same-run", "failed", 0);
        let mut existing_item = queue_item_fixture("same-run", "old-item", 0, "pending", None);
        existing_item.slug = "shared-task".to_string();
        state::replace_web_queue(&existing_run, &[existing_item]).unwrap();
        ...
        assert!(response.ok, "{}", response.output);
        let (_, items) = state::load_web_queue_run("same-run").unwrap();
        assert_eq!(items.len(), 1);
        assert_eq!(items[0].id, "new-item");
        assert_eq!(items[0].slug, "shared-task");
    }
