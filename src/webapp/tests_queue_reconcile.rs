...
mod queue_reconcile_tests {
    ...
    #[test]
    fn failed_graph_queue_retries_transient_remote_open_failure() {
        let _guard = test_support::env_guard();
        let temp = tempdir().unwrap();
        std::env::set_var("QCOLD_STATE_DIR", temp.path());
        let mut run = queue_run_fixture("graph-retry-open", "failed", 1);
        run.execution_mode = "graph".to_string();
        run.message = "failed to open remote managed task second through remote-dev-env".to_string();
        let first = queue_item_fixture(&run.id, "first", 0, "running", Some("agent-1"));
        let mut second = queue_item_fixture(&run.id, "second", 1, "failed", None);
        second.message = "failed to open remote managed task second through remote-dev-env: \
                          exit status: 255\nssh: connect to host 10.253.244.101 port 22: \
                          Connection timed out"
            .to_string();
        let third = queue_item_fixture(&run.id, "third", 2, "running", Some("agent-3"));
        let mut fourth = queue_item_fixture(&run.id, "fourth", 3, "pending", None);
        fourth.depends_on = vec![
            "first".to_string(),
            "second".to_string(),
            "third".to_string(),
        ];
        ...
        let Some((restarted_run, restarted_items)) =
            ...
            panic!("expected retryable open failure to restart");
        };
        ...
        assert!(queue_ready_item_ids(&restarted_run, &restarted_items)
            .contains(&"second".to_string()));
    }

    #[test]
    fn failed_graph_queue_does_not_retry_exhausted_remote_open_failure() {
        let _guard = test_support::env_guard();
        let temp = tempdir().unwrap();
        std::env::set_var("QCOLD_STATE_DIR", temp.path());
        let mut run = queue_run_fixture("graph-exhausted-open", "failed", 1);
        run.execution_mode = "graph".to_string();
        let mut item = queue_item_fixture(&run.id, "first", 0, "failed", None);
        item.attempts = queue_retry_limit();
        item.message = "failed to open remote managed task first through remote-dev-env: \
                        exit status: 255\nssh: connect to host 10.253.244.101 port 22: \
                        Connection timed out"
            .to_string();
        ...
        let (_, stored_items) = state::load_web_queue_run(&run.id).unwrap();
        assert!(!queue_run_needs_stale_reconcile(&run, &stored_items).unwrap());
        assert!(restart_resolved_failed_queue_run(&run, &stored_items)
            .unwrap()
            .is_none());
    }
