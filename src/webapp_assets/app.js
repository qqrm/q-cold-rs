const tg = window.Telegram && window.Telegram.WebApp;
    ...
    function render() {
      ...
      const nextCommand = `/agent_start ${track.value.trim()} :: codex exec "Use the launched agent workspace as your home base for ${root}. Start managed task ${slug.value.trim()} with cargo qcold task open ${slug.value.trim()}, enter that managed task worktree/devcontainer, reread AGENTS.md and task logs, then do: ${prompt.value.trim()} Drive the task to terminal closeout unless blocked, and return to the agent workspace after task closeout."`;
