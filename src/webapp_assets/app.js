const tg = window.Telegram && window.Telegram.WebApp;
    ...
    function render() {
      ...
      const nextCommand = `/agent_start ${track.value.trim()} :: codex exec "In ${root}, start managed task ${slug.value.trim()} with cargo qcold task open ${slug.value.trim()}, enter the managed task devcontainer, reread AGENTS.md and task logs, then do: ${prompt.value.trim()} Drive to terminal closeout unless blocked."`;
