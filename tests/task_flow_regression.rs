...
fn success_closeout_delivers_task_branch_to_primary_and_pushes_base() {
    ...
    assert!(stdout.contains("task-closeout-phase\tstart\tdeliver-to-primary"));
    ...
    assert!(listing.contains("metadata/terminal-receipt.env"));
    assert!(listing.contains("evidence/git-status.txt"));
