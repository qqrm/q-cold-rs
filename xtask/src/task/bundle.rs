...
fn render_terminal_summary(receipt: &TerminalReceipt<'_>, bundle: &Path) -> String {
    ...
    if let Some(message) = receipt.message.filter(|message| !message.trim().is_empty()) {
        push_summary_line(&mut summary, "Message", message.trim());
    }
...
fn render_terminal_receipt(receipt: &TerminalReceipt<'_>) -> String {
    ...
    for (key, value) in [
        ...
        ("MESSAGE", receipt.message.unwrap_or("").to_string()),
